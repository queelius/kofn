...
failed; the system lifetime T is the k-th order statistic of (T_1, …,
T_m). Observing T creates structured censoring on individual component
lifetimes.
...
- `flexhaz`: distribution prototypes
...
- `dist.structure`: DGP and topology (kofn delegates system density
  here)
...
for prototype subclasses (`dfr_exponential`, `dfr_weibull`) to be
available. The subclass dispatch is required for `kofn_subclass()` to
work.
...
[`dfr_exponential()`](https://queelius.github.io/flexhaz/reference/dfr_exponential.html),
[`dfr_weibull()`](https://queelius.github.io/flexhaz/reference/dfr_weibull.html)).
The S3 class of the prototype drives dispatch via `kofn_subclass()`.
...
`loglik(model)` -\> `function(df, par)` - `score(model)` -\>
`function(df, par)` - `fit(model)` -\> `function(df, par0, ...)` -
...
Ordered from least to most informative: - Scheme 0:
[`observe_right_censor()`](https://queelius.github.io/kofn/reference/observe_right_censor.md),
system lifetime only (black box) - Scheme 1: `observe_periodic(delta)`,
periodic inspection - Scheme 2:
[`observe_exact()`](https://queelius.github.io/kofn/reference/observe_exact.md),
complete monitoring (trivial)
...
- **Scheme 1** (`R/scheme1.R`): Periodic inspection likelihood for both
  exponential and Weibull. Resolves shape-scale ambiguity.
...
- `comp_lower_j, comp_upper_j`: component inspection intervals (Scheme
  1)

### Parameter conventions

- Exponential: `par = c(rate_1, ..., rate_m)`, m rates
- Weibull: `par = c(shape_1, scale_1, ..., shape_m, scale_m)`, 2m params
  interleaved
- All parameters must be positive
- Ascending parameter ordering for identifiability under permutation
  symmetry

### Component family dispatch

The `component` argument to
[`kofn()`](https://queelius.github.io/kofn/reference/kofn.md) is a
`dfr_dist` prototype from flexhaz. Family-specific S3 subclasses
(`"dfr_exponential"`, `"dfr_weibull"`) drive dispatch in
`kofn_subclass()`,
[`parse_params()`](https://queelius.github.io/kofn/reference/parse_params.md),
[`n_par_kofn()`](https://queelius.github.io/kofn/reference/n_par_kofn.md),
and the internal topology helpers. Adding a new supported family
requires: 1. One entry in `kofn_subclass()` mapping the dfr subclass to
a kofn subclass 2. S3 methods for loglik, score, hess_loglik, fit, rdata
on the new kofn subclass

## Key Mathematical Context

### k-out-of-n as censoring

For a k-out-of-n system with lifetime T = T\_{(k)} (k-th order
statistic): - 1 component observed exactly (the critical component) -
k-1 components left-censored at T (already failed) - n-k components
right-censored at T (still functioning)

### Exponential parallel (k=m)

- f_sys(t) = sum_j lambda_j \* exp(-lambda_j \* t) \* prod\_{i!=j} (1 -
  exp(-lambda_i \* t))
...
- f_sys(t) = sum_j f_j(t) \* prod\_{i!=j} F_i(t)
