...
\caption{Distribution of MAE across $R = 100$ replicates for the six
  ...
  The dashed line marks the symmetric-point MAE ($0.200$).  Component-level
  mechanisms (blue) achieve $3.5$--$6.9\times$ improvement over domain
  knowledge mechanisms (gray).}\label{fig:mechanism-comparison}
\end{figure}

The results confirm that component-level mechanisms dramatically outperform domain knowledge.  Component-level mechanisms
...
\caption{Autopsy coverage sensitivity ($m = 4$, $k = 2$, $n = 300$,
  ...
  91\% of the total improvement from no inspection to full
...
\caption{Median MAE vs.\ number of inspected components~$r$
  ...
  $r = 0$ to $r = 1$ captures 91\% of the total improvement,
...
$3.2\times$ improvement that captures 91\% of the total improvement
...
both median and mean MAE show slight fluctuation between $r = 3$ and
$r = 4$, within sampling variability.
...
periodic ($\MAE = 0.053$, $9.1\times$) outperforms masking
($\MAE = 0.099$, $4.8\times$), both far ahead of Scheme~0
($\MAE = 0.478$).  The larger improvement factors reflect the
